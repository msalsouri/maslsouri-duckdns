...
class AcceptStripePayments_Admin {
		...
		add_submenu_page( 'edit.php?post_type=' . ASPMain::$products_slug, __( 'Add-ons', 'stripe-payments' ), __( 'Add-ons', 'stripe-payments' ), $asp_addons_menu_capability, 'stripe-payments-addons', array( $this, 'display_addons_menu_page' ) );
		 ...
		 // Add this line to include the new submenu
                add_submenu_page(
                       ...
                       'manage_options',
                       'stripe-accounts',
                        array($this, 'render_stripe_accounts_page')
                );
	...
	public function render_stripe_accounts_page() {
            ?>
            <div class="wrap">
                <h1><?php _e('Manage Stripe Accounts', 'stripe-payments'); ?></h1>
                <form method="post" action="options.php">
                    <?php
                    settings_fields('stripe-accounts-group');
                    do_settings_sections('stripe-accounts');
                    submit_button();
                    ?>
                </form>
            </div>
	...
	public function register_settings( $value = '' ) {
		...
		add_settings_field(
			'disable_page_load_signature_check',
			__( 'Disable Page Load Signature Check', 'stripe-payments' ),
			array( $this, 'settings_field_callback' ),
			$this->plugin_slug . '-advanced',
			'AcceptStripePayments-experimental-settings',
			array(
				'field' => 'disable_page_load_signature_check',
				'desc'  => __( 'It is not recommended to disable the page load signature check. The signature check helps to prevent some bots from abusing your payment buttons.', 'stripe-payments' ),
			)
		);
	...
	public function settings_field_callback( $args ) {
		...
		switch ( $field ) {
			case 'checkbox':
				echo "<input type='checkbox' name='AcceptStripePayments-settings[" . esc_attr($field_name) . "]' value='1' " . ( $field_value ? 'checked=checked' : '' ) . " /><p class=\"description\">" . wp_kses_post( $desc ) . "</p>";
				break;
			case 'custom':
